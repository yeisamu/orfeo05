<?php
/************************************************************************
# PROYECTO: Orfeo   MODULO: Email - NEW.php        FECHA: Octubre 2012  *
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*
#                                                                       *
#                                                                       *
#                                                                       *
...
if($tipoMedio=="eMail"){
    ...
    if(!$asu) { 
        ...
        $mail_us1= $msg->header[$eMailMid]['fromaddress']." <".$msg->header[$eMailMid]['from'][0].">";
    }
}
...
?>
<head>
    <title>.:: Orfeo M&oacute;dulo de Radicaci&oacute;n::.</title>
...
if($Buscar){
    ...
    if(!$rs->EOF and is_numeric($busqueda)) {
        ...
        echo "<p><center><table width='90%' class=borde_tab celspacing=5><tr><td class=titulosError><center>No se han encontrado registros con numero de radicado <font color=blue>$nurad</font> <br>Revise el radicado escrito, solo pueden ser Numeros de 14 digitos <br><p><hr><a href=$ruta_raiz'/radicacion/edtradicado.php?fechaf=$fechaf&krd=$krd&drde=$drde'><font color=red>Intente de Nuevo</a></center></td></tr></table></center>";
        if(!$rsJHLC) die("<hr>");
    }
}
...
if($noradicar1)  $radicadopadre_exist=0;
?>
<script Language="JavaScript">
...
?>
            ...
            <td width="183" align="right" valign="bottom"> <input type="button" name="Button" value="BUSCAR" class="botones_funcion" onClick="Start($ruta_raiz.'/radicacion/buscar_usuario.php?krd=<?=$krd?>&nombreTp1=<?=$nombreTp1?>&nombreTp2=<?=$nombreTp2?>&nombreTp3=<?=$nombreTp3?>&busq_salida=<?=$busq_salida?>&ent=<?=$ent?>',1024,400);">
            </td>
        </tr>
        </table>
        </td>
...
?>
        ...
        ?>
        <center>
        <input class="botones_largo" value ="SUBIR IMAGEN DE FAX" type=button target= 'UploadFax' onclick="window.open($ruta_raiz.'/radicacion/uploadFax.php?<?=$varEnvio?>','Cargar Archivos de Fax', 'height=300, width=400,left=350,top=300')">
        </center>
        <?
        }
//  echo "<hr>$tipoMedio<hr>";
  ...
  ?>
  <center>
  <input class="botones_largo" value ="ASOCIAR EMAIL A RADICADO" type=button target= 'UploadFax' onclick="window.open($ruta_raiz.'/email/uploadMail.php?<?=$varEnvio?>','formulario', 'height=400, width=640,left=350,top=300')">
  </center>
  <?
 }
//echo "<script>window.open('radicado_n.php?nurad=$nurad&var_envio=$var_envio', 'ConfirmacionRad$nurad', 'height=260,width=430,left=350,top=300 ');</script>";
    ...
    $conexion = $db;
    error_reporting(0);
    include $ruta_raiz."/radicaciones/grb_direcciones.php";
    $verradicado = $nurad;

    echo "<script>window.open($ruta_raiz.'/verradicado.php?verrad=$nurad&var_envio=$var_envio".$datos_envio."&datoVer=985&ruta_raiz=".$ruta_raiz."', 'Modificaciï¿½n_de_Datos', 'height=700,width=650,scrollbars=yes');</script>";
    }
    echo  "<INPUT TYPE=HIDDEN NAME=nurad value=$nurad>";
...
<a href="<?echo $ruta_raiz?>/radicacion/hojaResumenRad.php?<?=$varEnvio?>" target="HojaResumen<?=$nurad?>" class=vinculos>Ver Hoja Resumen </a>
</u></b></font><br>
