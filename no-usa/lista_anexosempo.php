...
window.open(url,nombreventana,'height=660,width=640,scrollbars=yes,resizable=yes');
return;
}
...
<tr><td height="25" class="titulos4" colspan="10">GENERACION DE DOCUMENTOS </td>
...
if ($rs->fields["RADI_NUME_SALIDA"]==0 and $ruta_raiz != ".." and (trim($rs->fields["ANEX_CREADOR"])==trim($krd) OR $codusuario==1)
		)
		{
			...
			if($origen!=1  and $linkarchivo and $perm_borrar_anexo == 1 and $anexTipo == 4 )
	...
	{	if (!$rs->fields["RADI_NUME_SALIDA"])
		{	
			...
			// $ent = 9 Tipo de radicado PQR 
			if(($ent==2 or $ent==4 or $ent==7) && $puedeRadicarAnexo==1 )
			{	$rs->fields["SGD_PNUFE_CODI"]=0;
				...
				{	
					//By skina valido respuestas a doc de entrada 2,4,7
					if(($ent!=2 and $ent!=4 and $ent!=7) and $num_archivos==1 and !$rs->fields["SGD_PNUFE_CODI"] and $swRadDesdeAnex==false )
					{
    ...
    <font size=1>
		<?php
		 //Es un anexo de tipo tif (4) y el usuario tiene permiso para borrar tif
		if ( $origen!=1  and $linkarchivo and $perm_borrar_anexo == 1 and $anexTipo == 4 )
		{
