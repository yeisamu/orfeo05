...
// By skina, Nov 23/09	 and e.sgd_exp_estado !=2";
			...
			$sqlrad3.=" and r.radi_nume_radi = '$nurad' ORDER BY e.RADI_NUME_RADI";
...
//			$db->conn->debug=true;
			...
			//$queryDpto = "select distinct(d.dpto_nomb),d.dpto_codi from departamento d, sgd_eit_items i where d.dpto_codi=i.sgd_eit_dpto ORDER BY dpto_nomb";
			...
			if( !isset( $codDpto2 ) )
{
	$codDpto2 = 0;
			...
			//$queryMuni = "select distinct(m.MUNI_NOMB),m.MUNI_CODI FROM MUNICIPIO m , SGD_EIT_ITEMS i WHERE m.MUNI_CODI=i.sgd_eit_muni and DPTO_CODI='$codDpto2' ORDER BY MUNI_NOMB";
			...
			<td  class='titulos2'>EDIFICIO </td>
				...
				$sql="select sgd_eit_nombre,SGD_EIT_CODIGO from SGD_EIT_ITEMS where cast(codi_muni as varchar(5)) like '$codMuni2'
				and cast(codi_dpto as varchar(4)) like '$codDpto2' order by sgd_eit_nombre";
				$rs=$db->query($sql);
				...
				<?
			//modificado skina conversion de variables	
			$sql="select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as varchar(5)) like '$exp_edificio2' order by SGD_EIT_NOMBRE ";
			$rs=$db->query($sql);
			...
			<?
			//modificado skina conversion de  variables
			$sql="select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as varchar(5)) like '$exp_piso2' order by SGD_EIT_NOMBRE ";
			$rs=$db->query($sql);
			...
			?>
				...
				<? }
				//modificado skina conversion de variables
				$sql="select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as varchar(5)) like '$exp_item12' order by SGD_EIT_NOMBRE";
				$rs=$db->query($sql);
				...
				//modificado conversion de variable
			...
			<? }
			//modificado skina conversion de variables
			$sql="select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as varchar(5)) like '$exp_item22' order by SGD_EIT_NOMBRE";
			$rs=$db->query($sql);
			...
			?>
				...
				<? }
			//modificado skina conversion de variables
			$sql="select SGD_EIT_NOMBRE from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as varchar(5)) like '$exp_item31' order by SGD_EIT_NOMBRE";
			$rs=$db->query($sql);
			...
			if($item6[0]!=""){
				...
				//modificado skina conversion de variables
				$sql="select SGD_EIT_NOMBRE,SGD_EIT_CODIGO from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as varchar(5)) like '$exp_item31' order by SGD_EIT_NOMBRE";
				$rs=$db->query($sql);
...
//			*Modificado por Fabian Mauricio Losada
			...
			<TR>
				...
				$sqlrad1="select e.RADI_NUME_RADI,e.SGD_EXP_ESTADO,r.RADI_NUME_HOJA FROM SGD_EXP_EXPEDIENTE e, RADICADO r WHERE SGD_EXP_NUMERO LIKE '$num_expediente' and r.RADI_NUME_RADI=e.RADI_NUME_RADI and e.sgd_exp_estado !=2";
					...
					$sqlrad1.=" and e.RADI_NUME_RADI = '$nurad' 
							ORDER BY e.RADI_NUME_RADI";
//					$db->conn->debug=true;
...
//    	  $row = array();
  ...
  <div align="center">
    ...
    <span class='eerrores'><a href='../login.php' target=_parent><span class="textoOpcion">PorFavor intente validarse de nuevo. Presione aca !</span></a></span> 
	--></div>
</form>
           <?
		}
