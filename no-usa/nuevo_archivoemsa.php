...
{	$nivel=$rs->fields["CODI_NIVEL"];
	($tipo==0) ? $psql = " where  anex_tipo_codi<30 " : $psql=" ";
...
function doc_radicado()
{
	...
	else
	{
		...
		<?
		 ...
		 if (($verunico==1) && ($ent!=2 and $ent!=4 and $ent!=7)) { ?>
...
//podremos realizar combinaciÃ¯Â¿Â½n de correspondencia, ya que estos formatos permiten acceso al codigo.
//<br><i>El formato .doc no permite realizar este proceso. <a href='http://www.gnu.org/philosophy/no-word-attachments.es.html' target='softwarepropietario'>Mas InformaciÃ¯Â¿Â½n</a></i>
?>
			</td>
		</tr>
		<tr>
...
if($us_1 or $us_2 or $us_3)
{
	...
	$swDischekRad="";
	//by skina para emsa 230610	if (strlen(trim($radsalida))>0)	$swDischekRad = "disabled=true";
...
foreach ($tpNumRad as $key => $valueTp)
{
	...
	//Si se definiÃÂ³ prioridad en algÃÂºn tipo de radicaciÃÂ³n
...
<tr>
	...
	<tr>
		...
		<td colspan="3" class="listado2" align="center">
<?php
	/* Si viene la variable cc(Boton de destino copia) envia al modulo de grabaciÃ¯Â¿Â½n de datos
	 */
	$db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
	if($cc)
	{
...
<?php
	...
	$rs=$db->conn->Execute($isql);
	// $i_copias  Indica cuantas copias se han aÃ¯Â¿Â½adido
	$i_copias = 0;
	while($rs && !$rs->EOF)
	{
		$i_copias++;
