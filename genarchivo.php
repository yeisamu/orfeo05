...
if($radicar_documento)
{
	...
	//By skina Valido documentos de entrada 2,4,7
	// ent 9 es el tipo de radicado PQR
	//if(($ent!=2) and $nurad and $vpppp=="ddd")
	if(($ent!=2 and $ent!=4 and $ent!=7) and $nurad and $vpppp=="ddd")
	{
		$sec = $nurad;
...
//$db->conn->debug=true;
           ...
           if($rsUpdate) $estado= "Documento Impreso"; else $estado= "No Impreso";
//echo $estado;
...
<!--/*Modificacion Skina  05/11/2013*/
    ...
    /*Se comenta el formulario de   */
    /*marcar como impreso en la     */
    /*combinacion ODT para evitar   */
    /*conflictos con la comb en docx*/-->
<!--
...
} elseif ( $ext=="DOCX" || $ext=="docx" || $ext=="odt" || $ext == "ODT"){
...
?>
<!--<form id="MARCA" method="post">-->
<form id="MARCA" method="post" action="lista_anexos_seleccionar_transaccion.php?radicar=1&radicar_a=<?=$nurad?>&vp=n&<?="&".session_name()."=".trim(session_id()) ?>&radicar_documento=<?=$verrad?>&numrad=<?=$verrad?>&anexo=<?=$anexo?>&linkarchivo=<?=$linkarchivo?><?=$datos_envio?>&ruta_raiz=<?=$ruta_raiz?>&numfe=<?=$rs->fields['SGD_PNUFE_CODI']?>&tpradic=''&aplinteg=''&numextdoc=<?=$numextdoc?>" >
<p>
<input type="checkBox" name="marcar" aria-label="Casilla para marcra que el documento va a ser impreso" checked >
<input type="submit" class=e_buttons  value="Marcar Impresi&oacute;n" aria-label="Boton marcar documento como impreso">
</p>
...
?>
<br/>
<a href="./verradicado.php?PHPSESSID=172o16o0o154oJH<?=$krd?>verrad=<?=$rad_salida?>&menu_ver_tmp=2">Volver al radicado <?=$rad_salida?></a>
<br/>
<br/>
</body>
