	 ...
	 case 'mssql':
	  $isql = 'select
					 ...
					 where 
						...
						and '.$db->conn->substr.'(convert(char(15),b.radi_nume_radi), 5, '.$longitud_codigo_dependencia.')='.$dep_sel.'
	...
	case 'postgres':
	 $isql = 'select
					 ...
					 where 
					 	b.radi_nume_radi is not null
						and b.radi_nume_radi=d.radi_nume_radi
						and '.$db->conn->substr.'(CAST(b.radi_nume_radi AS varchar(15)), 5, '.$longitud_codigo_dependencia.')='."'$dep_sel'".'
						and b.tdoc_codi=c.sgd_tpr_codigo
						'.$whereTpAnulacion.'
						'.$whereFiltro.'
					  order by '.$order .' ' .$orderTipo;
