...
function informar( $radicados, $loginOrigen,$depDestino,$depOrigen,$codUsDestino, $codUsOrigen,$observa,$idenviador = null, $ruta_raiz="")
{
	...
	{	if (strstr($noRadicado,'-'))	$tmp = explode('-',$noRadicado);
		...
		if (strpos($record['RADI_NUME_RADI'], "'") === false) $record['RADI_NUME_RADI'] = "'".$record['RADI_NUME_RADI']."'";
		if (substr_count($record['RADI_NUME_RADI'], "'") < 2) $record['RADI_NUME_RADI'] = "'".str_replace("'", "", $record['RADI_NUME_RADI'])."'";
		...
		//Modificado idrd 
		if ($informaSql)        $tmp_rad[] = trim($record["RADI_NUME_RADI"],"'");
        }
                $depDestino = trim($depDestino, "'");
...
//Modificado por Fabian Mauricio Losada
  function archivar( $radicados, $loginOrigen,$depOrigen,$codUsOrigen,$observa)
  {
		...
		//$this->conn->Execute($isql);
		$this->db->conn->Execute($isql); # Ejecuta la busqueda
		$this->insertarHistorico(str_replace("'", "", $radicados),  $depOrigen , $codUsOrigen, '999', 1, $observa, 13);
		return $isql;
  }
...
function expm( $radicados, $loginOrigen,$depOrigen,$codUsOrigen,$observa)
  {
                $whereNivel = "";
                $radicadosIn = join(",",$radicados);
// Modificado por SkinaTech, se agrego la query y la variable $rad1 para q me tome el primer valor del arreglo.
                $rad1 = $radicados[0];
//              $this->db->conn->debug=true;
                $esql = "SELECT SGD_EXP_NUMERO
                 FROM   SGD_EXP_EXPEDIENTE
                 WHERE  RADI_NUME_RADI='$rad1'";
...
// By SkinaTech, 23 Nov de 2009
 // $this->insertarHistoricoExp( $_POST['numeroExpediente'], $radicados, $depOrigen, $codUsOrigen, $observa, 62 , 0 );
