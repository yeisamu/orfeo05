...
class Flujo{
	...
	function Flujo($db,$codProceso, $usuaDoc){
		if (isset($codProceso) && $codProceso != '')
			$this->codProceso = $codProceso;
		else 
			$this->codProceso = 0;
		$this->db = $db;
	...
	function aristasSiguiente($estadoActualExp){
		...
		ORDER BY SGD_FEXP_CODIGOINI,SGD_FEXP_CODIGOFIN";*/	
	       ...
	       // Se cambia la query ya que si se pasa como parametro el campo SGD_FEXP_CODIGOINI, da como resultado solo muestra la primera etapa del flujo 
	       // se necesita es que devuelva todos las etapas del flujo
	       // By Skina - jmgamez@skinatech.com

		$query = "SELECT * FROM SGD_FARS_FARISTAS 
                WHERE SGD_PEXP_CODIGO=$estadoActualExp 
                ORDER BY SGD_FEXP_CODIGOINI,SGD_FEXP_CODIGOFIN";
		$rs = $this->db->conn->query($query);
		$aristasArray = "";
		if($rs)
	 ...
	 */
	function cambioNodoExpediente($expNume,$radiNume,$nodoNuevo,$aristaCodi,$fldAutomatico,$observa,$proc){
	//	$this->db->conn->debug = true;

		$radiNume = explode(",", $radiNume);
		  for ($i=0;$i<count($radiNume);$i++) {
      			$radiNume[$i] = "'".$radiNume[$i]."'";
		...
		if($rs)
		{
			...
			$record["RADI_NUME_RADI"] = $radiNume[$i];
