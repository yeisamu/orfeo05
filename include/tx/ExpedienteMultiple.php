...
/** FUNCION CONSULTA EXPEDIENTE
  ...
  */
	function consulta_exp($radicado)
	{
		...
		{	case 'mssql':$radi_nume_radi="convert(varchar(11+".$longitud_codigo_dependencia."), e.radi_nume_radi)";
						break;
...
/**
 ...
 function insertar_expediente($num_expediente,$radicado,$depe_codi,$usua_codi,$usua_doc)
 {
	...
	$estado_expediente =0;
	$creadoOld = 0;
	$query="select * from SGD_SEXP_SECEXPEDIENTES e
			WHERE e.SGD_EXP_NUMERO='$num_expediente'";
	  $rs = $this->db->conn->query($query);
...
//		$cuentaEtapas=-1;
	...
	if($numExpediente){
	    ...
	    $query = "SELECT SGD_PEXP_DESCRIP, SGD_PEXP_CODIG, SGD_PEXP_TERMINOS, SGD_PEXP_AUTOMATICO
