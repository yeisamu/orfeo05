...
<!--  <td class=titulosError  width="80%" colspan="4" align="center">BUSQUEDA AVANZADA (SOLO PARA RADICADOS ARCHIVADOS) </td>-->
    <div id="titulo" style="width: 90%;" align="center"> Búsqueda avanzada (solo para radicados archivados)</div>

<tr><td width="20%" class='titulos5' align="left">
...
<td width="20%" class='listado2'>
	...
	?>
	<td width="20%" class='titulos5' align="left"><label for="buscar_exp"><label for="buscar_exp">EXPEDIENTE</label></label></td>
	<td width="20%" class='listado2'><input type=text name=buscar_exp value='<?=$buscar_exp?>' id="buscar_exp" class="tex_area" title="Buscar radicados por expediente">
	</td></tr>
	<tr><td width="20%" class='titulos5' align="left"><label for="tsub"><label for="tsub">SUBSERIE</label></label></td>
	<td width="20%" class='listado2'>
	<?
	...
	?>
	<td width="20%" class='titulos5' align="left"><label for="buscar_rad"><label for="buscar_rad">RADICADO</label></label></td>
	<td width="20%" class='listado2'><input id="buscar_rad" type=text name=buscar_rad value='<?=$buscar_rad?>' class="tex_area" title="Buscar por número de radicado">
	</td></tr>
	<tr><td width="20%" class='titulos5' align="left"><label for="cod_proc">PROCESO</label> </td>
	<td width="20%" class='listado2'>
          <?
...
<!--	<td width="20%" class='titulos2'><input type=text name=buscar_parametros value='<?=$buscar_parametros?>' class="tex_area">
   ...
   ?>
<td width="20%" class='titulos5'><label for="exp_edificio">EDIFICIO</label> </td>
<TD width="20%" class='listado2' >
		<? 
...
<tr><td width="20%" class='titulos5' align="left"><label for="buscar_piso"><?=$item1?></label> </td>
<td width="20%" class='listado2'>
		<?
...
<td width="20%" class='titulos5'><label for="buscar_ufisica"><?=$item2?></label> </td>
<td width="20%" class='listado2'>
		<?
...
<!--<td width="20%" class='titulos2'-->
	...
	//	if( $rsPE )
       // print $rsPE->GetMenu2("codDpto", $codDpto, "0:-- Seleccione --", false,""," onChange='submit()' class='select'" );
         ?>
     <tr>
         <td width="20%" class='listado1' align="left" colspan="2"> 
...
//		print $rsPX->GetMenu2("codMuni", $codMuni, "0:-- Seleccione --", false,"","class='select'" );
            ?>
         <td width="20%" class='listado1' colspan="2">
...
<!--</tr>-->
<tr >
    <td colspan="2" style="text-align: center;" class="listado1"><input type=submit value=Buscar name=Buscar class="botones" aria-label="Buscar archivos de acuerdo a criterios ingresados">&nbsp;</td>
<td colspan="2" style="text-align: center;" class="listado1"><a href='archivo.php?<?=session_name()?>=<?=trim(session_id())?>krd=<?=$krd?>'><input aria-label="Volver al menú del módulo de archivo" name='Regresar' align="middle" type="button" class="botones" id="envia22" value="Regresar"></td>
</table>
<br>
<?
if($Buscar){
...
if($buscar_rad!=""){$r="cast(e.RADI_NUME_RADI as varchar(15)) LIKE '%$buscar_rad%'";$b="and";$perm2=1;}
else {$r="";$b="";}
//if($codserie!='0'){$srds="s.SGD_SRD_CODIGO LIKE '$codserie'";$c="and";$perm2=1;} cambiado por error en tipo de datos proyecto eebp
if($codserie!='0'){$srds="s.SGD_SRD_CODIGO = $codserie";$c="and";$perm2=1;}
else {$srds="";$c="";}
//if($codiSBRD!='0'){$sbrds="s.SGD_SBRD_CODIGO LIKE '$codiSBRD'";$d="and";$perm2=1;} cambiado por error en tipo de datos proyceto eebp
...
//
if($buscar_ufisica!=""){
if($item2 ){$ufisica="e.SGD_EXP_UFISICA LIKE '$buscar_ufisica'";$t3="and";}
...
if($buscar_rete!=""){$foli="e.SGD_EXP_RETE = $buscar_rete";$k="and";$perm2=1;}
else {$foli="";$k="";}
