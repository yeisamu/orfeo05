...
if (!isset($pagina_sig)) $pagina_sig = ''; if (!isset($accion_sal)) $accion_sal = ''; if (!isset($selecdoc)) $selecdoc = ''; if (!isset($orderTipo)) $orderTipo = '';
if (!isset($orderNo)) $orderNo = ''; if (!isset($dependencia_busq1)) $dependencia_busq1 = '';
 ...
 if($busq_radicados_tmp)  {
   $where_fecha=" ";
    ...
    $where_fecha = " (.SGD_RENV_FECH >= ". $db->conn->DBTimeStamp($fecha_ini) ." and a.SGD_RENV_FECH <= ". $db->conn->DBTimeStamp($fecha_fin).") " ;
