...
//Modificado IDRD 25-abr-2008
        $q = "SELECT  dep.ID_CONT AS ID_CONT,
		dep.ID_PAIS AS ID_PAIS,		
		dep.ID_PAIS || '-' || dep.DPTO_CODI AS depto,
        	mun.ID_PAIS || '-' || mun.DPTO_CODI || '-' || mun.MUNI_CODI AS municip
		FROM DEPARTAMENTO dep, MUNICIPIO mun
		WHERE mun.DPTO_CODI = dep.DPTO_CODI and mun.id_cont=dep.id_cont and mun.id_pais=dep.id_pais ";

        if ($tipoCon == '0')
            $q .= "   and upper(dep.DPTO_NOMB) LIKE '%$consulta%'  ";
        else
            $q .= "   and upper(mun.MUNI_NOMB) LIKE '%$consulta%'  ";
        $rs = $db->query($q);
        unset($q);
        ?>
        <form action="consulta_depmuni.php?<?= $phpsession ?>&krd<?= $krd ?>&dependencia=<?= $dependencia ?>" method="post" enctype="multipart/form-data" name="formAdjuntarArchivos">
               <center>
        <div id="titulo" style="width: 55%;" align="center">Radicación masiva <br>consulta de la división política administrativa (divipola)</div>
            <table width='55%'  cellspacing="5"  align='center' class='borde_tab' border="1">
                <tr> 
<?php
if ($tipoCon == '0') {
    ?>
                        <td class="listado2" height="12" colspan="4"> 	
                            <BR>Resultado de b&uacute;squeda por Departamento: <?= $consulta ?><BR>
                        </td>
    <?php
} else {
    ?>
                        <td class="listado2" height="12" colspan="4"> 	
                            <BR>Resultado de b&uacute;squeda por Municipio: <?= $consulta ?><BR>
                        </td>
    <?php
}
?>
                ...
                ?>
                <tr align="center"> 
                    <td height="30" class="listado1" colspan="4">
                <center>
                    <input name="enviaPrueba" type="button"  class="botones" id="envia22"  onClick="document.formAdjuntarArchivos.action = 'menu_masiva.php?<?= $phpsession ?>&krd=<?= $krd ?>';
                            document.formAdjuntarArchivos.submit();" value="Cerrar">
