...
//error_log("####".print_r($_POST['check_value'],true)."####");
//echo "<pre>";
//echo print_r($_POST);
//echo "</pre>";
...
{ 	$nuir=$arrselctedForm[$i];
	if (strlen($nuir)>0)
	{	// si el nuir a analizar no esta en la selecci�n que lleg� finalmente
		//if (strpos($seleccTodos, trim($nuir).",")==false)
		if (in_array($nuir,explode(",",$seleccTodos))==false)
		{	//almacena los elementos excluidos de la selecci�n
