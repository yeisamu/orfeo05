    ...
    <table  width='95%' border="0" class='borde_tab'align="center">
        ...
        <form name='form1' action='cuerpo_exp.php?<?= session_name() . "=" . session_id() . "&krd=$krd&fechah=$fechah&$encabezado&$orno" ?>' method="post">
            ...
            <td rowspan="2" width='40%'>
                <label for="fechafi">Fecha Inclusion Expediente</label> <br>
                <label for="fechai">Inicial</label>
                <?
                if (!$fechai)
                    $fechai = fnc_date_calcm(date('Y-m-d'), '1');
                ?>
...
?>
            ...
            <td width='15%' align="left" rowspan="2">
            ...
            <td width='30%' align="rigth" rowspan="2">
                ...
                if ($tipo_archivo == 1) {
                    ...
                    <br> 
                    &nbsp;
                    <a href='../archivo/busqueda_archivo.php?<?= session_name() . "=" . session_id() . "&dep_sel=$dep_sel&krd=$krd&fechah=$fechah&$orno&adodb_next_page&nomcarpeta&tipo_archivo=$tipo_archivo&carpeta" ?>'> Busqueda Avanzada</a><br><br>
                <? } ?>

                <b><label for="dep_sel">Dependencia que pide el archivo del documento</label></b>
    ...
    <tr class="tablas" >
        <td width='20%'>    
            <label for="buscar_rad"> &nbsp;Radicado &nbsp&nbsp&nbsp&nbsp;</label>
    ...
    <tr>
        <td  align='left' height="40" class=titulos2 >Listar Por:
            <a href='cuerpo_exp.php?<?= $encabezado . $orno ?>&tipo_archivo=0' aria-label="Ordenar por documentos en espera de ser archivados"><span class='leidos'>
                    ...
                    ?>
                    Por Archivar</span></a>
            <? if ($tipo_archivo == 1) echo "$img7"; ?> <a href='cuerpo_exp.php?<?= $encabezado . $orno ?>&tipo_archivo=1' class="no_leidos" aria-label="Ordenar por documentos archivados"><span class='tpar'>
            <?
            if ($tipo_archivo == 1)
                echo "<b>";
            else
                                ...
                                while (!$rs->EOF) {
                                    if ($ki >= $registro and $ki < ($registro + 15)) {
                                        ...
                                        if ($rs->fields["SGD_TPR_CODIGO"] == 9999) {
                                            ...
                                            $dep_radicado = substr($rs->fields["RADI_NUME_RADI"], 4, $longitud_codigo_dependencia);
