...
//$tipoRadicado = $_SESSION['tipoRadicadoPqr'];
$errorFormulario = 0;
//$db->conn->debug = true;
		//Skina
                //Modificacion 
...
if($errorFormulario==0){
	...
	//echo $uploader->adjuntarYaSubidos();
...
//	}
	...
	if(!$_SESSION['dependencia']) $_SESSION['dependencia']=$_SESSION['depeRadicaFormularioWeb'];
	...
	if($tipoDocumento >= 0 && $tipoDocumento == 4 ){
        ...
        $_SESSION['radicado']= $radicado;
        $_SESSION['documento_destino']=$_GET['documento_destino'];
        if(!$_SESSION['dependencia']) $_SESSION['dependencia']= $depeRadicaFormularioWeb;
	...
	/*else {
		...
		$ins_ciu="insert into sgd_ciu_ciudadano values($tipdoc,".$num_ciu.",'".mb_strtoupper($_POST['nom_ciu'],"utf-8")."','".mb_strtoupper($_POST['direccion'],"utf-8")."','".mb_strtoupper($_SESSION['apell1_ciu'],"utf-8")."','','".$_SESSION['telefono']."','".$_SESSION['email']."',".$_SESSION['muni'].",".$_SESSION['depto'].",'".$_SESSION['doc_ciu']."')";
...
values ('$numeroRadicado', now(),".$_SESSION['tipo'].",".$_SESSION['mrec_codi'].",".$_SESSION['codigo_orfeo'].",to_date('".date('d')."/".date('m')."/".date('Y')."','dd/mm/yyyy'),'COLOMBIA',".$_SESSION['muni'].",".$carpeta.",".$_SESSION['depto'].",1,'". $descripcionAnexos ."', ";
	...
	,'".$_SESSION['depeRadicaFormularioWeb']."',1,5,1,0,0,".$radi_tp_deri.",0,0,0,0,".$tipoRadicado.")";
        ////Afecta el campo carp_per en la tabla usuario, si no esta en 0 no muestra la pqr en la bandeja del tipo de radicacion asignado
	//,'".$_SESSION['depeRadicaFormularioWeb']."',1,5,1,".$carpeta.",0,".$radi_tp_deri.",0,0,0,0,".$tipoRadicado.")";
	//inserta en radicado
...
$pdf->SetFont('Arial','',7);
$pdf->Text(110,48,textoPDF("Consulte su trámite en la página web http://www.sht.com.co"));
...
if ($_SESSION['email']!="" && $_SESSION['email']!="No registra") {
    //$usMailSelect  = $_SESSION["cuenta_mail"]; 
    $usMailSelect  = 'orfeo@sht.com.co'; 
    /*$mail->IsSMTP(); // telling the class to use SMTP
    $mail->AddReplyTo($usMailSelect);*/
    $mail->SetFrom($usMailSelect,"Sistema PQR'S ".$_SESSION['razonSocial']);
    ...
    $mail->Password   = "COLOCAR PW AQUI"; // SMTP account password
    $mail->Subject    = "Radicado " .$numeroRadicado.$_SESSION['razonSocial'];
    $mail->AltBody    = "Para ver el mensaje, por favor use un visor de E-mail compatible!";
    /*$url=true;*/
    $mail->AddAttachment("../bodega/$rutaPdf");
    $mail->AddAddress($_SESSION['email']);

    $asu .= "<hr>Sistema de gestion documental Orfeo. ".$_SESSION_['entidad_contacto']." ";
...
Para consultar su solicitud da click <a href=\'https://190.242.39.59/orfeo/consultaWeb/\' > aqu&iacute;</a>.<br><br>
<center>
<b>'.$_SESSION["entidad"].'</b><br>
Línea de atención:'.$_SESSION["entidad_tel"].' <br>
E-mail: '.$_SESSION["cuenta_mail"].'<br>'.$_SESSION["entidad_dir"].'<br><br>
Horario:
...
// Envio Correo de notificacion de radicacion a funcionario.
        $subject="Ha recibido PQR's desde la pagina Oficial de SHT";
        $cuerpo="
        <html>
        <head>
...
<div id="container" class="wufoo topLabel">
        ...
        <?php if($errorFormulario==0){?>
                ...
                <ul>
                        <div id="foli3">
			       <p>
                        	Su solicitud ha sido registrada de forma exitosa con el radicado No. <b><?=$numeroRadicado?></b> .
                       		Por favor tenga en cuenta estos datos para que realice la consulta del estado a su solicitud a través de la página web
                        	de la entidad.</br></br>
                        	
                               Pulse continuar para <b>finalizar la solicitud</b> y visualizar el documento en formato PDF. Si desea almacenelo
                               en su equipo o imprímalo.</p>
                        </div>
                        <div class="buttons">
                                <input type="button" name="Submit" value="Continuar" onclick="window.open('../bodega/<?=$rutaPdf?>')" />
                                <input type="button" name="Cerrar" value="Cerrar" onclick="window.location='http://www.sht.com.co'" />
                        </div>
                </ul>
