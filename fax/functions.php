	...
	function add_ticket($var_filename,$number,$dest,$text)
{
	...
	function doneq_list()
	{
		...
		unset($exec_output);
		unset($exec_return);
		$sshExec = "/usr/bin/faxstat -d | sort -t'|' +1n";
	...
	for ($i=0; $i < count($exec_output); $i++) {
    	if ( ereg($OUTPUT_REGEXP,$exec_output[$i]) ) {
        	...
        	print "        </td>\n";
        	print "        <td>\n";
        	print "        <a href=\"viewdq.php?var_jid=" . $dsp_jid . "\">Ver</a>\n";
	...
	function sendq_list()
	{
		...
		unset($exec_output);
		unset($exec_return);
		exec("/usr/bin/faxstat -s | sort",$exec_output,$exec_return);
		$no_queue = 0;
	for ($i=0; $i < count($exec_output); $i++) {
    	if ( ereg($OUTPUT_REGEXP,$exec_output[$i]) ) {
        print "       <tr align=\"center\"  class=listado2>\n";
	...
	function inbox_num()
	{
		exec("/usr/bin/faxstat -r | wc -l ",$exec_output,$exec_return);
		if($exec_output[0] > 0)
		{
	...
	function recvq_list()
	{
		...
		exec("/usr/bin/faxstat -r | sort",$exec_output,$exec_return);         
		$iClass = 2;
		for ($i=0; $i < count($exec_output); $i++) {
			...
			if($iClass == 1) $iClass =2; else $iClass =1;
				...
				//Modificado skinatech 30-10-08 para mssql
				$iSql= " SELECT
							USUA_LOGIN
							,SGD_RFAX_FECH as  SGD_RFAX_FECH
...
function faxStat_list()
	{
		...
		print("</tr>");
  		print("<tbody>");
		exec("/usr/bin/faxstat -d | grep -v '|'",$exec_output,$exec_return);         
