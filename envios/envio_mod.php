...
<html>
    <head>
        <title>Modificacion envío</title>
...
?>
    ...
    <body>
        ...
        ?>
    <center>
                 <div id="titulo" style="width: 85%;" align="center">Modificación envío de documentos</div>
        ...
        <form name='forma' action='envio_mod.php?<?= session_name() . "=" . session_id() . "&krd=$krd&fecha_h=$fechah&dep_sel=$dep_sel&estado_sal=$estado_sal&estado_sal_max=$estado_sal_max&no_planilla=$no_planilla" ?>' method="post">
            ...
            if (!$reg_envio) {
                ?>
                <table border=2 width=85% class=borde_tab cellspacing="5">
                    <!--DWLayoutTable-->
                    <tr class='titulos2' >
			...
			";
                                ...
                                if (!$reg_envio) {
                                    if ((!$direccion_us or ! $destino)) {
                                        ...
                                        ?>
                                        <input name=reg_envio type=button value='Modificar registro de envío' onClick="modificar_reg();" class='botones_largo'>
                                        <input name=reg_envio type=hidden value='Modificar registro de envío' >
                                        <?
                                    }
                                }
                                ?>
