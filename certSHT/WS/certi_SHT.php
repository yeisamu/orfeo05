<?php
    ...
    function genRadi($category,$name,$idPerson,$idPlace,$contractTerm,$position,$admissionDate,$salary,$bfc,$requestOf,$commissions) {
        // Valido campos comunes
        if ( empty($name) || empty($idPerson) || empty($idPlace) || empty($contractTerm) || empty($position) || empty($admissionDate) ) {
             return "Esta omitiendo campos comunes obligatorios para la generación de la certificación";
        }

        // Dependiendo la certificacion validamos los valores
        switch ($category) {
            case 1:
               if ( empty($salary) || empty($bfc) ) {
                   return "Esta omitiendo campos obligatorios para la generación de la certificación seleccionada: Tipo $category";
               }
               ...
               break;
            case 2:
               if ( empty($salary) || empty($requestOf) ) {
                   return "Esta omitiendo campos obligatorios para la generación de la certificación seleccionada: Tipo $category";
               }
               // Definiendo formato
               $typeCert="tipoII.odt";
	...
	// Combine correspondencia
        ...
        $command = sprintf($command, "pdf", "/var/www/pruebas/bodega/".substr($datosRad['path'],0,-3)."odt");
