...
{ 	if (${'s'.$i})		//Si se envio el vector....
	{
		...
		{	$codDepto=$rs->fields['IDPAIS'].'-'.$rs->fields['IDDPTO'];
			if (strlen(trim($codDepto))==0)	$codDepto="NO REGISTRA";

			$codMuni=$codDepto.'-'.$rs->fields['IDMPIO'];

			if (strlen(trim($codMuni))==0)	$codMuni="NO REGISTRA";

			$depto->departamento_codigo($codDepto);
			$muni->municipio_codigo($codDepto,$codMuni);
			$nombDepto=$depto->get_dpto_nomb();
			if (strlen($nombDepto)==0)	$nombDepto="NO REGISTRA";

			$nombMuni=$muni->get_muni_nomb();
			if (strlen($nombMuni)==0)	$nombMuni="NO REGISTRA";
			...
			else $nombPais = "NO REGISTRA";

			$nombESP = $rs->fields['PPAL'];
			if (strlen(trim($nombESP))==0)	$nombESP="NO REGISTRA";

			$dirESP = $rs->fields['DIR'];
			if (strlen(trim($dirESP))==0)	$dirESP="NO REGISTRA";

			$nuir =  $rs->fields['ID'];
			if (strlen(trim($nuir))==0)	$nuir="NO REGISTRA";

			$nomRepESP = $rs->fields['DESCR'];
			if (strlen(trim($nomRepESP))==0)	$nomRepESP="NO REGISTRA";
			...
			//$tmp_rs = $this->cursor->query("SELECT CTT_NOMBRE,CTT_CARGO FROM SGD_DEF_CONTACTOS WHERE CTT_ID=$tmp_var");
			if($tmp_rs)
